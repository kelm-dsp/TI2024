...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define VCNL4000_ADDRESS 0x13<<1
/* USER CODE END PD */
...
volatile uint32_t adc_dma_result[100];
int adc_channel_count = sizeof(adc_dma_result)/sizeof(adc_dma_result[0]);
uint8_t adc_conv_complete_flag = 0;
uint32_t dma_result_buffer[100];
static uint32_t piston1;
uint16_t value;
//static uint32_t piston2;
//static uint32_t piston3;
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	/*
	writeVCNLByte(hi2c2, 0x84, 0x0F); // Configures ambient light measures - Single conversion mode, 128 averages
	writeVCNLByte(hi2c2, 0x83, 15); // sets IR current in steps of 10mA 0-200mA --> 200mA
	writeVCNLByte(hi2c2, 0x89, 2); // Proximity IR test signal freq, 0-3 - 781.25 kHz
	writeVCNLByte(hi2c2, 0x8A, 0x81); // proximity modulator timing - 129, recommended by Vishay
	//
	writeVCNLByte(hi2c3, 0x84, 0x0F); // Configures ambient light measures - Single conversion mode, 128 averages
	writeVCNLByte(hi2c3, 0x83, 15); // sets IR current in steps of 10mA 0-200mA --> 200mA
	writeVCNLByte(hi2c3, 0x89, 2); // Proximity IR test signal freq, 0-3 - 781.25 kHz
	writeVCNLByte(hi2c3, 0x8A, 0x81); // proximity modulator timing - 129, recommended by Vishay
	*/
  /* USER CODE END 2 */
